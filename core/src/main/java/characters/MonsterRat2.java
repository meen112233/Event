...
public class MonsterRat2 {
    ...
    public MonsterRat2(final World world, final float x_px, final float y_px) {
        this.world = world;
    ...
    private Body initPhysicsBody(World world, float x, float y) {
        ...
        GameScreen.bodies.put(body, "monsterRat2");
    ...
    public  void update(int delta) {
        ...
        if(movelr >= 0 && movelr <= 70){
            checkmove = false;
        }else if(movelr >= 200 && movelr <= 210){
            checkmove = true;
        }
        if(checkmove == true){
            state = State.WALKL;
